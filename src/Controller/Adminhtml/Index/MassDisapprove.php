...
class MassDisapprove extends AbstractMassAction implements HttpPostActionInterface
{
    ...
    {
        $testimonialsDisapproved = 0;
        foreach ($collection->getAllIds() as $id) {
            $testimonial = $this->testimonialFormRepository->getById($id);
            $testimonial->setStatus(TestimonialForm::STATUS_DISAPPROVED);
            $testimonial->setVisible(false);
            $this->testimonialFormRepository->save($testimonial);
            $testimonialsDisapproved++;
        }

        if ($testimonialsDisapproved) {
            $this->messageManager->addSuccessMessage(__('A total of %1 record(s) were disapproved.', $testimonialsDisapproved));
        }
